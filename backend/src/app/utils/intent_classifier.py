...
def _build_dynamic_prompt_parts() -> tuple[str, list[dict[str, str]]]:
    """Delegate to `app.tools.classify_tool._build_dynamic_prompt_parts`.

    Kept as a thin wrapper for backward compatibility so other modules
    that import this function continue to work.
    """
    from app.tools.classify_tool import _build_dynamic_prompt_parts as _builder
...
async def _classify_with_litert_lm(message: str, context: dict | None = None) -> dict[str, Any] | None:
    ...
    try:
        ...
        min_threshold = float(getattr(settings, "litert_lm_min_confidence_threshold", 0.5))
        ...
        return {
            ...
            "model": getattr(settings, "litert_lm_intent_model", ""),
        }
...
class IntentClassifier:
    ...
    async def classify_detailed(message: str, context: dict | None = None) -> dict[str, Any]:
        ...
        # Prefer on-device LiteRT classification when the classify agent is
        # configured to use the 'litert' provider. The explicit
        # `settings.litert_lm_enabled` flag was removed; infer intent from
        # per-agent provider configuration instead. Fall back to LLMs when
        # LiteRT classification is unavailable or not configured.
        use_litert = False
        try:
            agent_cfg = getattr(settings, "classify_agent", None)
            if agent_cfg and getattr(agent_cfg, "provider", "").lower() == "litert":
                use_litert = True
        except Exception:
            use_litert = False
        ...
        if use_litert:
            ...
            if not getattr(settings, "litert_lm_fallback_to_llm", True):
                return {
        ...
        if settings.main_llm_provider in ("openai", "strands"):
            intent_model = (settings.classify_model or "").strip() or settings.main_model
        elif settings.main_llm_provider == "litert_lm":
            intent_model = (
                (settings.classify_model or "").strip()
