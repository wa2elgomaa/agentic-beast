...
async def generate_json_object(
    *,
    ...
    """
    provider = (settings.main_llm_provider or "").strip().lower()
    ...
    if provider == "ollama":
        url = f"{settings.main_model_base_url.rstrip('/')}/api/chat"
        payload = {
            "model": model,
            "format": "json",
            "stream": False,
    ...
    raise RuntimeError(
        f"Unsupported AI_PROVIDER '{settings.main_llm_provider}'. "
