"""Simple v1 CodingAgent implementation.
...
class CodingAgentSchema(PydanticBaseModel):
    """Extract person information from text."""
    response_text: str = Field(description="Response from the coding agent")
    response_json: Dict[str, Any] = Field(description="Structured JSON response from the coding agent")

class CodingAgent:
    """A callable agent that handles classification requests.

    Usage:
        agent = CodingAgent(provider_factory=ProviderFactory())
    ...
    async def execute(self, context: Optional[Dict[str, Any]]) -> CodingAgentSchema:
        ...
        return self.agent.structured_output(
            output_model=CodingAgentSchema,
            prompt=message,
        )
...
def get_agent() -> CodingAgent:
    """Factory helper to instantiate a CodingAgent with default ProviderFactory."""
    return CodingAgent()
